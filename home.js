...
function findStr(){
	for(let i = 0; i < deals.length; i++){
		if(deals[i].title.includes('15')){
			//.replace method returns a
			//new string, does not alter
			//original string in the
			//array above
			let newTitle = deals[i].title.replace(/15/g, "10");	
...
//		console.log(deals[i].desc);
		if(deals[i].desc.includes("March")){
			//.replace method returns a
			//new string, does not alter
			//original string in the
			//array above
			let newDesc = deals[i].desc.replace(/March/g, "April").trim();
			console.log(newDesc);
			return true;
		}
	}
